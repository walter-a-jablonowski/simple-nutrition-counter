...
$backupZip = defined('DEPLOY_BACKUP_ZIP') ? DEPLOY_BACKUP_ZIP : false;
$python    = defined('DEPLOY_PYTHON') ? DEPLOY_PYTHON : 'python';
...
echo "- $backupDir\n";
echo "- as " . ( $backupZip ? 'a single zip archive' : 'a folder' ) . " (DEPLOY_BACKUP_ZIP)\n";
...
try {
  ...
  if( $backupDir === '' )
    ...
    backup( array_map( fn( $item ) => "$destDir/$item", $backup),
      $backupDir, "$destDir/", $backupZip, $python
    );
